...
{
	[InputControlLayout(stateType = typeof(GameCubeControllerStateUSB), isGenericTypeOfDevice = true)]
	public class GameCubeControllerUSB : InputDevice, IDualMotorRumble
	{
		/// <summary>
		...
		/// </summary>
		public static GameCubeControllerUSB current { get; private set; }
		...
		{
			//       var mayflashAdapter = new InputDeviceMatcher()
			//.WithInterface("HID")
			//.WithCapability("vendorId", 0x0079);
			//       InputSystem.RegisterLayout<GameCubeController>(matches: mayflashAdapter);

			var adapter = new InputDeviceMatcher()
				.WithInterface("HID")
				//.WithCapability("vendorId", 0x057E).WithProduct("0x0337");
				.WithCapability("vendorId", 0x0079)
				.WithCapability("productId", 0x6);
				//.WithProduct("0x0337");
				//.WithProduct("0x6");
			InputSystem.RegisterLayout<GameCubeControllerUSB>(matches: adapter);
		}
